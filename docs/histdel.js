// HISTDEL v1: GitHub deletion engine + deletion history viewer ("histdel").
// Every remote delete asks for a loud confirmation, snapshots the file first when
// possible, commits the DELETE, and records it so you can see — and restore — later.
(function(){
  if(window.__vbHistDel)return;
  window.__vbHistDel=true;
  ...
  function del(path){
    ...
    window.api("GET","/repos/"+c.repo+"/contents/"+path.split("/").map(encodeURIComponent).join("/")+"?ref="+c.branch)
      ...
      .then(function(){
        return commit([{kind:"DELETE",path:path}],"studio: delete "+path);
      })
      .then(function(cm){
    ...
    commit(paths.map(function(p){return {kind:"DELETE",path:p}}),"studio: delete folder "+folder+" ("+paths.length+" files)")
      .then(function(cm){
        paths.forEach(function(p){
          record({path:p,repo:c.repo,branch:c.branch,sha:cm.sha,ts:Date.now(),content:null,restored:null,folder:folder});
        });
  ...
  function renderList(){
    ...
    a.forEach(function(ent){
      ...
      top.appendChild(p);
      if(ent.restored){
        var rb=E("span","","restored");
        rb.style.cssText="color:#3fb950;font-size:9px;font-weight:800";
        top.appendChild(rb);
      }
      row.appendChild(top);
